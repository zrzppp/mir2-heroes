...
//内存映射类字符串资源
//const
  SCannotResize = '内存映射 %s 已被另一进程打开,现在不能做修改内存映射文件大小的操作';
  SCreateMapError = '不能建立映射文件';
  SCreateMapViewError = '不能建立映射试图';
  SDefaultMapName = 'Map';
  SEOFError = '无法读到文件尾';
  SFileOpenError = '无法打开文件 %s';
  SMapCloseError = '无法关闭映射句柄';
  SNameChangeWhileActiveError = '映射激活时不能更改映射文件名';
  SReadOnly = '映射文件 %s 当前为只读状态 ';
  SResizeAnywayQuery = '继续操作将丢失映射文件中的所有内容. 是否继续?';
  SResizeOpenWarning = '提示: 试图更改已打开的映射文件尺寸';
  SResizeWhileInactiveError = '映射没有激活时不能更改映射文件尺寸';
