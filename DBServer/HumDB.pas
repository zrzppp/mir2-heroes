...
type
  ...
  private
    ...
    function UpdateRecord(nIndex: Integer; HumRecord: pTHumInfo; boNew: Boolean): Boolean;
    function DeleteRecord(nIndex: Integer): Boolean;
    function GetRecord(n08: Integer; HumDBRecord: pTHumInfo): Boolean;
    function GetRecordX(n08: Integer; HumDBRecord: pTHumInfo): Boolean;
  public
    constructor Create();
    ...
    procedure Close();
    function Index(const sName: string): Integer;
    function Get(n08: Integer; HumDBRecord: pTHumInfo): Integer;
    function GetX(n08: Integer; HumDBRecord: pTHumInfo): Integer;
    function GetBy(n08: Integer; HumDBRecord: pTHumInfo): Boolean;
    function FindByName(const sChrName: string; ChrList: TList): Integer;
    function FindByAccount(const sAccount: string; var ChrNameList: TList): Integer;
    function ChrCountOfAccount(const sAccount: string): Integer;
    function HeroCountOfAccount(const sAccount: string): Integer;
    function Add(HumRecord: pTHumInfo): Boolean;
    function Delete(const sName: string): Boolean;
    function Update(nIndex: Integer; HumDBRecord: pTHumInfo): Boolean;
    function UpdateBy(nIndex: Integer; HumDBRecord: pTHumInfo): Boolean;
  end;
  ...
  private
    ...
    function GetRecordX(nIndex: Integer; HumanRCD: pTHumDataInfo): Boolean;
    function UpdateRecord(nIndex: Integer; HumanRCD: pTHumDataInfo; boNew: Boolean): Boolean;
    function DeleteRecord(nIndex: Integer): Boolean;
  public
    constructor Create;
    destructor Destroy; override;
    ...
    function GetX(nIndex: Integer; HumanRCD: pTHumDataInfo): Integer;
    function Update(nIndex: Integer; HumanRCD: pTHumDataInfo): Boolean;
    function Add(HumanRCD: pTHumDataInfo): Boolean;
    function Find(const sChrName: string; List: TStrings): Integer;
    procedure Rebuild();

    function Delete(const sChrName: string): Boolean; overload;
...
function TFileHumDB.GetCount(): Integer;
begin
  Result := m_QuickList.Count;
end;

procedure TFileHumDB.LoadFormFile(const FileName: string);
begin
  if boHumDBReady then Exit;
...
begin
  for I := 0 to m_QuickIDList.Count - 1 do begin
    ChrList := TList(m_QuickIDList.Objects[I]);
    ...
    if Open then begin
      ...
      if FileRead(m_nFileHandle, DBHeader, SizeOf(TDBHeader)) = SizeOf(TDBHeader) then begin
        ...
        for nIndex := 0 to DBHeader.nHumCount - 1 do begin
          ...
          {if (Pos('传说',DBRecord.sChrName)>0) or (Pos('传说',DBRecord.Header.sName)>0)then begin
            MainOutMessage(Format('boDeleted:%s nSelectID:%d boIsHero:%s sChrName:%s sAccount:%s boDeleted:%s',
            [BooleanToStr(DBRecord.Header.boDeleted),
            DBRecord.Header.nSelectID,
            BooleanToStr(DBRecord.Header.boIsHero),
            DBRecord.sChrName,
            DBRecord.sAccount,
            BooleanToStr(DBRecord.boDeleted)
            ]));
          end;}
...
function TFileHumDB.OpenX(): Boolean;
begin
  //Lock();
  n4 := 0;
  Result := False;
  ...
  if FileExists(m_sDBFileName) then begin
    if m_FileStream = nil then
      m_FileStream := TFileStream.Create(m_sDBFileName, fmOpenRead or fmShareDenyNone);
    Result := m_FileStream <> nil;
  end;
end;

procedure TFileHumDB.CloseX();
begin
  if Assigned(m_FileStream) then FreeAndNil(m_FileStream);
...
function TFileHumDB.Index(const sName: string): Integer;
begin
  Result := m_QuickList.GetIndex(sName);
end;

function TFileHumDB.Get(n08: Integer;
  HumDBRecord: pTHumInfo): Integer;
var
  nIndex: Integer;
begin
...
function TFileHumDB.GetX(n08: Integer;
  HumDBRecord: pTHumInfo): Integer;
var
  nIndex: Integer;
begin
  if (n08 >= 0) and (n08 < m_QuickList.Count) then begin
    nIndex := Integer(m_QuickList.Objects[n08]);
    if GetRecordX(nIndex, HumDBRecord) then Result := nIndex
    else Result := -1;
  end else Result := -1;
end;

function TFileHumDB.GetRecordX(n08: Integer;
  HumDBRecord: pTHumInfo): Boolean;
begin
  if m_FileStream.Seek(SizeOf(THumInfo) * n08 + SizeOf(TDBHeader), soBeginning) <> -1 then begin
    m_FileStream.Read(HumDBRecord^, SizeOf(THumInfo));
    n4 := n08;
    Result := True;
  end else Result := False;
end;

function TFileHumDB.FindByName(const sChrName: string;
  ChrList: TList): Integer;
var
  I, nCount: Integer;
begin
  nCount := 0;
  for I := 0 to m_QuickList.Count - 1 do begin
    if CompareLStr(m_QuickList.Strings[I], sChrName, Length(sChrName)) then begin
      ChrList.Add(m_QuickList.Objects[I]);
      Inc(nCount);
    end;
  end;
  Result := nCount;
...
function TFileHumDB.FindByAccount(const sAccount: string;
  var ChrNameList: TList): Integer;
begin
  Result := m_QuickIDList.GetChrList(sAccount, ChrNameList);
end;

function TFileHumDB.ChrCountOfAccount(const sAccount: string): Integer;
var
  ChrList: TList;
  I, n18: Integer;
...
function TFileHumDB.HeroCountOfAccount(const sAccount: string): Integer;
var
  ChrList: TList;
  I, n18: Integer;
  HumDBRecord: THumInfo;
...
function TFileHumDB.Add(HumRecord: pTHumInfo): Boolean;
var
  Header: TDBHeader;
  nIndex: Integer;
begin
...
function TFileHumDB.UpdateRecord(nIndex: Integer; HumRecord: pTHumInfo;
...
function TFileHumDB.Delete(const sName: string): Boolean;
var
  n10: Integer;
  HumRecord: THumInfo;
  ChrNameList: TList;
...
function TFileHumDB.Update(nIndex: Integer; HumDBRecord: pTHumInfo): Boolean;
begin
  Result := False;
...
function TFileHumDB.UpdateBy(nIndex: Integer; HumDBRecord: pTHumInfo): Boolean;
begin
  Result := False;
...
procedure TFileDB.LoadFormFile(const FileName: string);
begin
  if boDataDBReady then Exit;
  m_sDBFileName := FileName;
  m_sIdxFileName := FileName + '.idx';
  n4ADAE4 := 0;
  n4ADAF0 := 0;
  m_nLastIndex := -1;
...
function TFileDB.LoadDBIndex: Boolean;
var
    ...
    if Open then begin
      ...
      if FileRead(m_nFileHandle, DBHeader, SizeOf(TDBHeader)) = SizeOf(TDBHeader) then begin
        ...
        for nIndex := 0 to DBHeader.nHumCount - 1 do begin
         ...
         {if Pos('传说',RecordHeader.sName)>0 then begin    //
            MainOutMessage(Format('boDeleted:%s nSelectID:%d boIsHero:%s sChrName:%s',
            [BooleanToStr(RecordHeader.boDeleted),
            RecordHeader.nSelectID,
            BooleanToStr(RecordHeader.boIsHero),
            RecordHeader.sName
            ]));
          end;}
...
function TFileDB.OpenX(): Boolean;
begin
  //Lock();
  n4 := 0;
  Result := False;
...
function TFileDB.Index(const sName: string): Integer;
begin
  Result := m_QuickList.GetIndex(sName);
end;

function TFileDB.GetX(nIndex: Integer;
  HumanRCD: pTHumDataInfo): Integer;
var
  nIdx: Integer;
begin
  if (nIndex >= 0) and (nIndex < m_QuickList.Count) then begin
    nIdx := Integer(m_QuickList.Objects[nIndex]);
    if GetRecordX(nIdx, HumanRCD) then Result := nIdx
    else Result := -1;
  end else Result := -1;
end;

function TFileDB.GetRecordX(nIndex: Integer;
  HumanRCD: pTHumDataInfo): Boolean;
begin
  if m_FileStream.Seek(SizeOf(THumDataInfo) * nIndex + SizeOf(TDBHeader), soBeginning) <> -1 then begin
    m_FileStream.Read(HumanRCD^, SizeOf(THumDataInfo));
    n4 := nIndex;
    Result := True;
  end else Result := False;
end;
...
function TFileDB.UpdateRecord(nIndex: Integer;
  HumanRCD: pTHumDataInfo; boNew: Boolean): Boolean;
var
  nPosion, n10: Integer;
  dt20: TDateTime;
...
function TFileDB.Find(const sChrName: string;
...
function TFileDB.Delete(const sChrName: string): Boolean;
var
  n10: Integer;
begin
  Result := False;
