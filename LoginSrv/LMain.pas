...
//    nRandomCode: Integer;
  ...
  {TMakeAccount = record
    ...
    dwClientTick: LongWord;
    dwMakeAccountTick: LongWord;
    dwLoginTick: LongWord;
  end;
  pTMakeAccount = ^TMakeAccount;  }
...
procedure SendGateAddIPToGate(Socket: TCustomWinSocket; sUserIPaddr: string);

procedure SessionAdd(Config: pTConfig; sAccount, sIPaddr, sServerName: string; nSessionID: Integer; boPayCost, bo11: Boolean);
...
procedure AccountQueryServerName(Config: pTConfig; UserInfo: pTUserInfo); //查询服务器列表
...
procedure SendIPToGate(sUserIPaddr: string);
var
  FrmMain: TFrmMain;

implementation

uses IDDB, MasSock, FrmFindId, HUtil32, EncryptUnit, GateSet,
...
begin
  ...
  Config.MakeAccountList := TQuickList.Create;
...
begin
  ...
  try
    ...
    UserInfo.dwClientTick := GetTickCount();
    UserInfo.dwStartTick := GetTickCount();
    UserInfo.dwMakeAccountTick := GetTickCount();
    UserInfo.bo60 := False;
...
begin
  ...
  DefMsg := MakeDefaultMsg(SM_RANDOMCODE, Random(888888888), 0, 0, 0);
  sRandomCode := EncryptString256(IntToStr(UserInfo.nRandomCode), IntToStr(DefMsg.Recog));
...
begin
  try
    ...
    case DefMsg.Ident of
      {CM_RANDOMCODE: begin
          if Config.boRandomCode then begin
            SendRandomCode(UserInfo);
          end;
        end; }
      CM_QUERYSERVERNAME: begin //查询服务器列表
      ...
      CM_IDPASSWORD: begin
          if (UserInfo.sAccount = '') and UserInfo.boSelServer then begin
...
begin
  try
    ...
    if (sUserEntryMsg <> '') and (sUserAddEntryMsg <> '') then begin
      ...
      if (Length(UserEntry.sAccount) < 4) or (Length(UserEntry.sPassword) < 4) then begin
        bo21 := False;
        nErrCode := -2;
      end;
            ...
            if n10 < 0 then begin
              ...
              if UserEntry.sAccount <> '' then begin
                if AccountDB.Add(DBRecord) then begin
                  Config.MakeAccountList.Lock;
                  try
                    if Config.MakeAccountList.GetIndex(UserEntry.sAccount) < 0 then begin
                      Config.MakeAccountList.AddObject(UserEntry.sAccount, TObject(UserInfo.dwClientTick));
                    end;
                  finally
...
function GetMakeAccountTick(CreateDate: TDateTime): LongWord;
var
  nInt64: Int64;
  wYear, wMonth, wDay: Word;
  wNYear, wNMonth, wNDay: Word;
begin
  try
    DecodeDate(Now, wYear, wMonth, wDay);
    DecodeDate(CreateDate, wNYear, wNMonth, wNDay);
    if (wYear = wNYear) and (wMonth = wNMonth) and (wDay = wNDay) then begin
      nInt64 := SecondsBetween(Now, CreateDate);
      Result := LongWord(nInt64 * 1000);
    end else begin
      Result := 60 * 1000 * 60;
    end;
  except
  ...
  try
        ...
        if (n10 >= 0) and (AccountDB.Get(n10, DBRecord) >= 0) then begin
          ...
          if (DBRecord.nErrorCount < 5) or ((GetTickCount - DBRecord.dwActionTick) > 60000) then begin
            if DBRecord.UserEntry.sPassword = sPassword then begin
              ...
              //MainOutMessage('AccountLogin:' + IntToStr(UserInfo.dwMakeAccountTick));
              nCode := 1;
            end else begin
              Inc(DBRecord.nErrorCount);
              DBRecord.dwActionTick := GetTickCount();
    ...
    if nCode = 1 then begin
     ...
     { if not UserInfo.boPayCost then begin
      ...
      GetSelGateInfo(Config, UserInfo.sServerName, Config.sGateIPaddr, sSelGateIP, nSelGatePort);
      ...
      if (sSelGateIP <> '') and (nSelGatePort > 0) then begin
        ...
        if not UserInfo.boPayCost then begin
          DefMsg := MakeDefaultMsg(SM_PASSOK_SELECTSERVER, 0, 0, 0, 0);
        end else begin
          DefMsg := MakeDefaultMsg(SM_PASSOK_SELECTSERVER,
            nIDCost,
            LoWord(nIPCost),
      ...
      {SendGateMsg(UserInfo.Socket,
        ...
        FrmMasSoc.SendServerMsg(SS_OPENSESSION, UserInfo.sServerName,
          UserInfo.sAccount + '/' + IntToStr(UserInfo.nSessionID) + '/' + IntToStr(Integer(UserInfo.boPayCost)) + '/' + IntToStr(nPayMode) + '/' + UserInfo.sUserIPaddr + '/' + IntToStr(UserInfo.dwMakeAccountTick));
...
begin
  if (UserInfo.sAccount = '') then begin
    UserInfo.boSelServer := False;
    DefMsg := MakeDefaultMsg(SM_SERVERNAME, 0, 0, 0, Config.ServerNameList.Count);
...
begin
  ...
  if (UserInfo.sAccount = '') and (sServerName <> '') and (not IsLogin(Config, UserInfo.nSessionID)) then begin
    GetSelGateInfo(Config, sServerName, Config.sGateIPaddr, sSelGateIP, nSelGatePort);

    {
    MainOutMessage('Config.sGateIPaddr:'+Config.sGateIPaddr);
    MainOutMessage('sSelGateIP:'+sSelGateIP);
    MainOutMessage('nSelGatePort:'+IntToStr(nSelGatePort));
    }
    if (sSelGateIP <> '') and (nSelGatePort > 0) then begin
      ...
      if FrmMasSoc.IsNotUserFull(sServerName) then begin
        ...
        nIndex := Config.MakeAccountList.GetIndex(UserInfo.sAccount);
        if nIndex >= 0 then begin
          Config.MakeAccountList.Lock;
          try
            UserInfo.dwMakeAccountTick := LongWord(Config.MakeAccountList.Objects[nIndex]);
            Config.MakeAccountList.Delete(nIndex);
          finally
            Config.MakeAccountList.UnLock;
          end;
        end;
...
procedure SessionAdd(Config: pTConfig; sAccount, sIPaddr, sServerName: string;
...
procedure SendGateAddIPToGate(Socket: TCustomWinSocket;
  sUserIPaddr: string);
var
  sSendMsg: string;
begin
...
begin
  ...
  //Config.boRandomCode := LoadConfigBoolean(sSectionServer, sIdentRandomCode, Config.boRandomCode);
