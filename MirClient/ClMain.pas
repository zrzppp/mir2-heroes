...
begin
  ...
  while True do begin
    ...
    if not FileExists(sDirectory + flname) then begin
      DScreen.AddChatBoardString('[屏幕截图: ' + flname + ']', clGreen, clWhite);
      Break;
    end;
        ...
        if ssCtrl in Shift then begin
          ...
          //
            if (GetTickCount - g_dwLatestStruckTick > 10000) and
              ...
              AppExit;
            end else DScreen.AddChatBoardString('你目前正在战斗中不能离开..', clyellow, clRed);
          end;
        end;
      end;
    Word('B'): begin //英雄挂机
          ...
          case g_QueryBagItem of
            q_QueryHum: begin
                ...
                if g_MySelf <> nil then begin
                  ...
                  DScreen.AddChatBoardString('正在刷新包裹...', clWhite, clFuchsia);
                end;
              end;
            q_QueryHero: begin
                if g_MyHero <> nil then begin
                  ...
                  DScreen.AddChatBoardString('正在刷新英雄包裹...', clWhite, clFuchsia);
                end else begin
                  g_QueryBagItem := q_QueryHum;
                  if g_MySelf <> nil then begin
                    g_boQueryHumBagItem := True;
...
begin
 // if target<>nil then
  ...
  if (nHitMsg = CM_LONGHIT) and g_Config.boSmartWalkLongHit then begin //走位刺杀
  ...
  if (nHitMsg = CM_LONGHIT) and g_Config.boSmartPosLongHit and (not target.m_boDeath) and TargetInSwordLongAttackRange(tdir, target, 2) then begin //隔位刺杀
...
begin
  ...
  DScreen.AddChatBoardString('装备升级成功！', clWhite, clRed);
...
begin
 ...
 // if sMapName <> g_sMapName then begin
  //end else begin
    if (abs(g_MySelf.m_nCurrX - nX) <= 1) and (abs(g_MySelf.m_nCurrY - nY) <= 1) then begin
      LegendMap.Stop;
      DScreen.AddChatBoardString(Format('自动移动坐标点(%d:%d)已到达', [nX, nY]), GetRGB(154), clWhite);
    end else begin
      ...
      LegendMap.EndX := nX;
      LegendMap.EndY := nY;
      DScreen.AddChatBoardString(Format('自动移动至坐标(%d:%d)，点击鼠标任意键停止...', [nX, nY]), GetRGB(154), clWhite);
    end;
  end;
end;
...
begin
  ...
  if g_boQueryHumBagItem then
    DScreen.AddChatBoardString('刷新包裹成功...', clWhite, clFuchsia);
end;
...
begin
  ...
  if g_boQueryHeroBagItem then
    DScreen.AddChatBoardString('刷新英雄包裹成功...', clWhite, clFuchsia);
end;
...
begin
  ...
  if (LegendMap.EndX > 0) and (LegendMap.EndY > 0) then begin
    if (abs(g_MySelf.m_nCurrX - LegendMap.EndX) <= 1) and (abs(g_MySelf.m_nCurrY - LegendMap.EndY) <= 1) then begin
      DScreen.AddChatBoardString(Format('自动移动坐标点(%d:%d)已到达', [LegendMap.EndX, LegendMap.EndY]), GetRGB(154), clWhite);
      LegendMap.Stop;
    end else begin
