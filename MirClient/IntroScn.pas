...
begin
  ...
  if Sender = m_EdSSNo then begin
   { FrmDlg.NAHelps.Add('请输入右边的验证码');
    FrmDlg.NAHelps.Add('');
    FrmDlg.NAHelps.Add('如果验证码显示为0可能是');
    FrmDlg.NAHelps.Add('获取验证码失败，请关闭');
    FrmDlg.NAHelps.Add('本窗口重新打开');  }
...
begin
  ...
  {for I := Low(DelCharArray) to High(DelCharArray) do begin
    DelCharArray[I].sChrName := '人物' + IntToStr(I);
...
begin
  ...
  if g_boGetRandomBuffer then begin //打开验证码窗口
