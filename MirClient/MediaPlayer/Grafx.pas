...
procedure FillGradRect( DC: HDC; AX, AY, AX0, AY0: Longint; ColorStart, ColorStop: TColor; SmoothFactor: TSmoothFactor = 1 );
procedure FillGradDiag( DC: HDC; AW, AH: Longint; ColorStart, ColorStop: TColor; SmoothFactor: TSmoothFactor = 1 );
...
fgLoop:
  if BGradX then
    ...
    N := Y - EndY;
  Inc( N );
  RedInt := Longint( GetRValue( CF ) ) shl 16;                  { Setup color differences }
  GreenInt := Longint( GetGValue( CF ) ) shl 16;
  BlueInt := Longint( GetBValue( CF ) ) shl 16;

  if ( N > 0 ) then
  begin
    RedInc := ( ( Longint( GetRValue( CI ) ) shl 16 ) - RedInt ) div N;
    GreenInc := ( ( Longint( GetGValue( CI ) ) shl 16 ) - GreenInt ) div N;
    BlueInc := ( ( Longint( GetBValue( CI ) ) shl 16 ) - BlueInt ) div N;
  end
  else
  begin
    RedInc := 0;
...
procedure FillGradRect( DC: HDC; AX, AY, AX0, AY0: Longint; ColorStart, ColorStop: TColor; SmoothFactor: TSmoothFactor );
var
  C, CI, CF, CPrev: TColor;
  ...
  XInc, YInc: Longint;
  XDWord: TDWordWord;
  X: Longint absolute XDWord;
  YDWord: TDWordWord;
  Y: Longint absolute YDWord;
  N: Longint;
  R, H, V: TRect;
  Brush: HBrush;
  Q, Q0: Longint; { Quality (i.e. speed) }
begin
...
procedure FillGradDiag( DC: HDC; AW, AH: Longint; ColorStart, ColorStop: TColor; SmoothFactor: TSmoothFactor );
var
  C, CI, CF, CPrev: TColor;
  BlueDWord: TDWordColor;
  GreenDWord: TDWordColor;
