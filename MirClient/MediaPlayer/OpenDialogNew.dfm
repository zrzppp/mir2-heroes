object frmOpenDiaLogNew: TfrmOpenDiaLogNew
  Left = 710
  Top = 429
  BorderIcons = [biSystemMenu]
  BorderStyle = bsDialog
  Caption = #26597#25214#24433#35270#25991#20214
  ClientHeight = 216
  ClientWidth = 441
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 12
  object RzLabel1: TRzLabel
    Left = 8
    Top = 160
    Width = 60
    Height = 12
    Caption = #25991#20214#21517'(&N):'
  end
  object RzLabel2: TRzLabel
    Left = 8
    Top = 184
    Width = 72
    Height = 12
    Caption = #25991#20214#31867#22411'(&T):'
  end
  object RzLabel3: TRzLabel
    Left = 8
    Top = 8
    Width = 48
    Height = 12
    Caption = #25628#23547'(&I):'
  end
  object RzShellCombo: TRzShellCombo
    Left = 120
    Top = 8
    Width = 185
    Height = 22
    ShellList = RzShellList
    TabOrder = 0
  end
  object RzShellList: TRzShellList
    Left = 8
    Top = 32
    Width = 425
    Height = 113
    FileFilter = 
      '*.mpg;*.mp3;*.mp4;*.avi;*.wav;*.rm;*.rmvb;*.dat;*.wmv;*.3gp;*.mp' +
      'eg'
    IconOptions.AutoArrange = True
    ReadOnly = True
    TabOrder = 1
    ViewStyle = vsList
    OnDblClickOpen = RzShellListDblClickOpen
    OnSelectItem = RzShellListSelectItem
  end
  object EditFileName: TRzEdit
    Left = 88
    Top = 160
    Width = 233
    Height = 20
    TabOrder = 2
  end
  object ComboBoxFileType: TRzComboBox
    Left = 88
    Top = 184
    Width = 233
    Height = 20
    ItemHeight = 12
    TabOrder = 3
    Text = '('#24433#35270#25991#20214')'
    Items.Strings = (
      '('#24433#35270#25991#20214')')
    ItemIndex = 0
  end
  object RzButtonOpen: TRzButton
    Left = 344
    Top = 160
    Height = 20
    Caption = #25171#24320'(&O)'
    TabOrder = 4
    OnClick = RzButtonOpenClick
  end
  object RzButtonClose: TRzButton
    Left = 344
    Top = 184
    Height = 20
    Caption = #21462#28040
    TabOrder = 5
    OnClick = RzButtonCloseClick
  end
  object RzBitBtnUp: TRzBitBtn
    Left = 328
    Top = 4
    Width = 30
    TabOrder = 6
    OnClick = RzBitBtnUpClick
    Glyph.Data = {
      
      5C37ACE8E8E8E8E8E8E8E8E8E8E8E8E8AC81ACE8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
end
