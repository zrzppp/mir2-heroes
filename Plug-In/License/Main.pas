...
//CpuNo：决定了获得第几个cpu内核的第几个序列号。
  ...
  var
    CPU: TCPU;
  begin
    SetCPU(GetCurrentProcess, 1); //第一个cpu的第一个
    CPU := TCPU.Create;
    try
      CPU.GetInfo();
          ...
          case RegistryConfig.RegistryStatus of
            ...
            r_Date: begin
                if Date > RegistryConfig.RegistryDate then begin
                  boOpenDlg := True;
                end else RegistryInfo.RegistrySuccess := R_SystemConfig.RegistrySuccess;
              end;
            r_Forever: RegistryInfo.RegistrySuccess := R_SystemConfig.RegistrySuccess;
          end;
          ...
          if boUpdate then begin
            ...
            if WriteRegKey(1, R_MySubKey, R_Key, sKey) then begin
              ...
              if (not boOpenDlg) and Assigned(R_SystemConfig.RegistryInfo) then begin
                ...
                RegistryInfo.RegistrySuccess := R_SystemConfig.RegistrySuccess;
                Exit;
              end;
            end else begin
              boOpenDlg := True;
