...
var
  ...
  g_Config: TConfig = (
    ...
    GatePort: 7100;
    GateAddr: '0.0.0.0';
    nMaxConnOfIPaddr: 1000;
    dwKeepConnectTimeOut: 60 * 1000 * 5;
  ...
  try
    ...
    {  g_SessionList.Lock;
      ...
      for nIndex := 0 to g_SessionList.MaxCount - 1 do begin
        ...
        nCheckCode := 8;
        if (Session <> nil) and (Session.Socket <> nil) and (Session.Socket.Connected) then begin
          if GetTickCount - Session.dwConnctCheckTick > g_Config.dwKeepConnectTimeOut then begin
            nCheckCode := 9;
            Session.Socket.Close; //¿ÕÁ¬½Ó
            nCheckCode := 10;
            Continue;
          end;
          nCheckCode := 11;
