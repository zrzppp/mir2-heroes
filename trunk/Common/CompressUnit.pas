...
procedure CompressBuffer(const InData: Pointer; InSize: LongInt; out OutData: Pointer; out OutSize: LongInt);
procedure UnCompressBuffer(const InData: Pointer; InSize: LongInt; out OutData: Pointer; out OutSize: LongInt);

procedure CompressBufferL(const InData: Pointer; InSize: LongInt; out OutData: Pointer; out OutSize: LongInt);
procedure UnCompressBufferL(const InData: Pointer; InSize: LongInt; out OutData: Pointer; out OutSize: LongInt);
...
procedure CompressStream1(InStream, OutStream: TStream; InSize: LongInt);
procedure DeCompressStream1(InStream, OutStream: TStream); }
...
var
  ...
  OutBuffer: BufferPtr;
  PBits: Pointer;
  CompressedSize, BytesRead, InSizePos, OutSizePos: LongInt;
begin
  InSizePos := 0;
  OutSizePos := 0;
  OutSize := 0;
  ...
  while InSize > 0 do begin
    if InSize > SizeOf(BufferArray) then begin
      BytesRead := SizeOf(BufferArray);
    end else begin
      BytesRead := InSize;
    end;
    InBuffer := Pointer(Integer(InData) + InSizePos);

    ReallocMem(OutData, OutSize + SizeOf(BufferArray) + SizeOf(Integer));

    OutBuffer := Pointer(Integer(OutData) + OutSizePos + SizeOf(Integer));
    CompressedSize := Compression(InBuffer, OutBuffer, BytesRead);

    PBits := Pointer(Integer(OutData) + OutSizePos);
    Move(CompressedSize, PBits^, SizeOf(Integer));

    OutSizePos := OutSizePos + SizeOf(Integer) + CompressedSize;
    OutSize := OutSize + SizeOf(Integer) + CompressedSize;

    InSizePos := InSizePos + BytesRead;
...
begin
  ...
  OutSize := 0;
  OutData := nil;
  while InSizePos < InSize do begin
    PBits := Pointer(Integer(InData) + InSizePos);
    InSizePos := InSizePos + SizeOf(Integer);
    Move(PBits^, CompressedSize, SizeOf(Integer));
    InBuffer := Pointer(Integer(InData) + InSizePos);

    ReallocMem(OutData, OutSize + SizeOf(BufferArray));
    OutBuffer := Pointer(Integer(OutData) + OutSize);
    UnCompressedSize := DeCompression(InBuffer, OutBuffer, CompressedSize);

    OutSize := OutSize + UnCompressedSize;
    InSizePos := InSizePos + CompressedSize;
  end;
end;
...
begin
  ...
  BytesRead := InStream.Read(InData^, InSize);

  CompressBuffer(InData, BytesRead, OutData, OutSize);
  if OutData <> nil then begin
    OutStream.Seek(0, soBeginning);
    OutStream.WriteBuffer(OutData^, OutSize);
    FreeMem(OutData);
  end;
  FreeMem(InData);
end;
...
begin
  ...
  UnCompressBuffer(InData, InSize, OutData, OutSize);
  if OutData <> nil then begin
    OutStream.Seek(0, soBeginning);
    OutStream.WriteBuffer(OutData^, OutSize); //Buffer
    FreeMem(OutData);
  end;
  FreeMem(InData);
end;
