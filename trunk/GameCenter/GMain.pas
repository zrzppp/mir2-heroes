...
uses GShare, HUtil32, Grobal2, GLoginServer, EncryptUnit, DataBackUp;
{$R *.dfm}
...
begin
  ...
  if (g_nServerVersion >= 0) and (g_nUpDateVersion >= 0) then begin
    case g_nServerVersion of
      0: MainOutMessage('当前是分身版M2Server，版本号:' + IntToStr(g_nUpDateVersion));
      1: MainOutMessage('当前是英雄版M2Server，版本号:' + IntToStr(g_nUpDateVersion));
    end;
