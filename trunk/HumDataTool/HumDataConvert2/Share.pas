...
interface
uses
  Windows, Classes, SysUtils, Controls, ComCtrls, RzLabel, RzPanel, RzRadChk, RzStatus;
const
  TESTMODE = 0;
type
  TConvertData = class
...
const
  ...
  g_sFileNames:string = 'Mir.db';
var
  g_boClose: Boolean;

  g_sSourceFiles: string;
...
begin
  {try
    if m_ProgressBar <> nil then begin
      if m_ProgressBar.Max then
      
      Inc(m_nPercent);
      m_ProgressBar.Percent := Trunc(m_nPercent / (m_nCount / 100));
    end;
  except
  end;  }
end;
