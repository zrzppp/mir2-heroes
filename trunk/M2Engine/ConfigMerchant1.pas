unit ConfigMerchant1;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, Spin, ObjNpc, Envir;
...
type
  ...
  private
    ...
    procedure ModValue();
    procedure uModValue();
    procedure RefListBoxMerChant(Envir: TEnvirnoment);
    procedure ClearMerchantData(Envir: TEnvirnoment);
...
begin
  ...
  SelMerchant := nil;
  RefListBoxMerChant(SelEnvirnoment);

  boOpened := True;
  Caption := SelEnvirnoment.sMapDesc;
  ShowModal;
end;
...
begin
  if Application.MessageBox(PChar('是否确认清除NPC临时数据？'), '确认信息', MB_YESNO + MB_ICONQUESTION) = mrYes then begin
    ClearMerchantData(SelEnvirnoment);
  end;
end;
...
procedure TfrmConfigMerchant.ClearMerchantData(Envir: TEnvirnoment);
var
  I: Integer;
  Merchant: TMerchant;
begin
  ...
  try
    for I := 0 to UserEngine.m_MerchantList.Count - 1 do begin
      Merchant := TMerchant(UserEngine.m_MerchantList.Items[I]);
      if Merchant.m_PEnvir = SelEnvirnoment then begin
        Merchant.ClearData();
      end;
    end;
  finally
...
procedure TfrmConfigMerchant.RefListBoxMerChant(Envir: TEnvirnoment);
var
  I: Integer;
  Merchant: TMerchant;
begin
