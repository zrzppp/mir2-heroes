...
uses
  ...
  Dialogs, ComCtrls, StdCtrls, Spin, Grids, ExtCtrls, Grobal2;
...
type
  ...
  TfrmGameConfig = class(TForm)
    ...
    CheckBoxRobotAttack: TCheckBox;
    Label149: TLabel;
    EditRobotAttackTime: TSpinEdit;
    TabSheet11: TTabSheet;
    ButtonClientlSave: TButton;
    GroupBox73: TGroupBox;
    CheckBoxCanOldClientLogon: TCheckBox;
    CheckBoxCheckClientVersion: TCheckBox;
    GroupBox78: TGroupBox;
    CheckBoxAllowClientChgSpeed: TCheckBox;
    GroupBox79: TGroupBox;
    CheckBox1: TCheckBox;
    ButtonRefClientConfig: TButton;
    GroupBoxInfo: TGroupBox;
    Label16: TLabel;
    EditSoftVersionDate: TEdit;
    RadioGroupShowClientItemStyle: TRadioGroup;
    GroupBox80: TGroupBox;
    Label150: TLabel;
    EditUseItemSpeedTime: TSpinEdit;
    GroupBox81: TGroupBox;
    Label151: TLabel;
    EditMaxLevel: TSpinEdit;
    GroupBox82: TGroupBox;
    Label152: TLabel;
    EditGreenNumber: TSpinEdit;
    GroupBox76: TGroupBox;
    ScrollBarWalkSpeedRate: TScrollBar;
    GroupBox83: TGroupBox;
    ScrollBarAttackSpeedRate: TScrollBar;
    GroupBox84: TGroupBox;
    CheckBoxCheckSpeedHack: TCheckBox;
    procedure EditRobotAttackTimeChange(Sender: TObject);
    ...
    procedure CheckBoxRobotAttackClick(Sender: TObject);
    procedure ButtonClientlSaveClick(Sender: TObject);
    procedure CheckBoxCanOldClientLogonClick(Sender: TObject);
    procedure CheckBoxCheckClientVersionClick(Sender: TObject);
    procedure CheckBoxAllowClientChgSpeedClick(Sender: TObject);
    procedure ButtonRefClientConfigClick(Sender: TObject);
    procedure RadioGroupShowClientItemStyleClick(Sender: TObject);
    procedure EditUseItemSpeedTimeChange(Sender: TObject);
    procedure EditMaxLevelChange(Sender: TObject);
    procedure EditGreenNumberChange(Sender: TObject);
    procedure ScrollBarWalkSpeedRateChange(Sender: TObject);
    procedure ScrollBarAttackSpeedRateChange(Sender: TObject);
    procedure CheckBoxCheckSpeedHackClick(Sender: TObject);
  private
    boOpened: Boolean;
    boModValued: Boolean;
...
uses EncryptUnit, Common, M2Share, HUtil32, SDK, ActionSpeedConfig;
...
var
  ...
  dwTickTime: LongWord;
begin
  EditMaxLevel.MaxValue := High(LongInt);
...
begin
  ...
  ButtonClientlSave.Enabled := True;
end;
...
begin
  ...
  ButtonClientlSave.Enabled := False;
end;
...
begin
  ...
  dwOldNeedExps := g_Config.dwNeedExps;
  for I := 1 to GridLevelExp.RowCount - 1 do begin
  ...
  CheckBoxRobotAttack.Checked := g_Config.boRobotAttack;
  EditRobotAttackTime.Value := g_Config.nLimitLoginTime;

  CheckBoxCanOldClientLogon.Checked := g_Config.boCanOldClientLogon;
  CheckBoxCheckClientVersion.Checked := g_Config.boCheckClientVersion;
  CheckBoxAllowClientChgSpeed.Checked := g_Config.boAllowClientChgSpeed;

  //RadioGroupClientOptionType.ItemIndex := g_Config.nClientGameOptionType - 1;

  RadioGroupShowClientItemStyle.ItemIndex := g_Config.nShowClientItemStyle;

  EditMaxLevel.Value := g_Config.nMaxLevel;

  EditGreenNumber.Value := g_Config.nGreenNumber;


  ScrollBarWalkSpeedRate.Position := g_Config.nWalkSpeedRate;
  ScrollBarAttackSpeedRate.Position := g_Config.nAttackSpeedRate;

  CheckBoxCheckSpeedHack.Checked := g_Config.boCheckSpeedHack;
  RefGameVarConf();
  RefCharStatusConf();
...
begin
  ...
  EditUseItemSpeedTime.Value := g_Config.nUseItemSpeed;
end;
...
procedure TfrmGameConfig.EditMaxLevelChange(Sender: TObject);
var
  nMaxLevel: Integer;
begin
  if not boOpened then Exit;

  nMaxLevel := MinLong(EditMaxLevel.Value, High(LongInt));

  EditMaxLevel.Value := nMaxLevel;
  g_Config.nMaxLevel := nMaxLevel;
  ModValue();
end;
...
var nOldVerstionDate: Integer;
begin
  if not boOpened then Exit;
  GetExVersionNO(CLIENT_VERSION_NUMBER, nOldVerstionDate);
  g_Config.nSoftVersionDate := Str_ToInt(EditSoftVersionDate.Text, nOldVerstionDate);
  ModValue();
end;
...
begin
  ...
  if OldLevelExpScheme <> LevelExpScheme then begin
    for I := 1 to 1000 do begin
      if dwOldNeedExps[I] <> g_Config.dwNeedExps[I] then
...
procedure TfrmGameConfig.EditRobotAttackTimeChange(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.nLimitLoginTime := EditRobotAttackTime.Value;
  ModValue();
end;
...
procedure TfrmGameConfig.CheckBoxRobotAttackClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boRobotAttack := CheckBoxRobotAttack.Checked;
  ModValue();
end;

procedure TfrmGameConfig.ButtonClientlSaveClick(Sender: TObject);
begin
...
procedure TfrmGameConfig.CheckBoxCheckClientVersionClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boCheckClientVersion := CheckBoxCheckClientVersion.Checked;
  ModValue();
end;

procedure TfrmGameConfig.CheckBoxAllowClientChgSpeedClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boAllowClientChgSpeed := CheckBoxAllowClientChgSpeed.Checked;
  ModValue();
end;

procedure TfrmGameConfig.ButtonRefClientConfigClick(Sender: TObject);
begin
  UserEngine.RefServerConfig;
end;

procedure TfrmGameConfig.RadioGroupShowClientItemStyleClick(
  Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.nShowClientItemStyle := RadioGroupShowClientItemStyle.ItemIndex;
  ModValue();
end;

procedure TfrmGameConfig.EditUseItemSpeedTimeChange(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.nUseItemSpeed := EditUseItemSpeedTime.Value;
  ModValue();
end;

procedure TfrmGameConfig.EditGreenNumberChange(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.nGreenNumber := EditGreenNumber.Value;
  ModValue();
end;

procedure TfrmGameConfig.ScrollBarWalkSpeedRateChange(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.nWalkSpeedRate := ScrollBarWalkSpeedRate.Position;
  ModValue();
end;

procedure TfrmGameConfig.ScrollBarAttackSpeedRateChange(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.nAttackSpeedRate := ScrollBarAttackSpeedRate.Position;
  ModValue();
end;

procedure TfrmGameConfig.CheckBoxCheckSpeedHackClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boCheckSpeedHack := CheckBoxCheckSpeedHack.Checked;
  ModValue();
end;
