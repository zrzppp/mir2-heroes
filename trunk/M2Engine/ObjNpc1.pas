    ...
    if (g_SellList.GetSellListByItemName(sItemName, List20) >= 0) and (List20 <> nil) and (List20.Count > 0) then begin
      ...
      if StdItem <> nil then begin
        ...
        for II := nIndex to List20.Count - 1 do begin
          ...
          ItemUnit.GetItemAddValue(@SellOffInfo.UseItems, StdItem20);
    ...
    if (g_SellList.GetSellListByItemName(sItemName, List20) >= 0) and (List20 <> nil) and (List20.Count > 0) then begin
      ...
      if StdItem <> nil then begin
        ...
        for II := List20.Count - 1 downto 0 do begin
          ...
          ItemUnit.GetItemAddValue(@SellOffInfo.UseItems, ClientItem.s);

          ClientItem.s.AddValue := SellOffInfo.UseItems.AddValue;
          ClientItem.s.AddPoint := SellOffInfo.UseItems.AddPoint;
          ClientItem.s.MaxDate := SellOffInfo.UseItems.MaxDate;
          ...
          if PlayObject.IsEnoughBag then begin
            ...
            if g_SellList.Delete(SellOffInfo, True) then begin
              if g_GoldList.Add(AddSellOffInfo) then begin
                New(UserItem);
                UserItem^ := AddSellOffInfo.UseItems;
                PlayObject.m_ItemList.Add(UserItem);
                PlayObject.SendAddItem(UserItem);
  ...
  case cMethod of
    ...
    '-': begin
        if BaseObject.m_Abil.Exp > LongWord(nExp) then begin
          Dec(BaseObject.m_Abil.Exp, LongWord(nExp));
        end else begin
          BaseObject.m_Abil.Exp := 0;
        end;
      end;
    '+': begin
        BaseObject.GetExp(LongWord(nExp));
        if BaseObject.m_btRaceServer = RC_PLAYOBJECT then begin
  ...
  try
    for I := 0 to g_AICharNameList.Count - 1 do begin
      ...
      if (UserEngine.GetPlayObject(sCharName) = nil) and (not UserEngine.FindAILogon(sCharName)) then begin
        UserEngine.AddAILogon(sMapName, sCharName, nX, nY, GetTickCount + nC * 500);
        nC := nC + 1;
      end;
...
begin
  if PlayObject.m_MyGuild <> nil then begin
    ...
    if (nMemberMaxLimit < 0) then begin
      VarInfo := GetVarValue(PlayObject, QuestActionInfo.sParam2, sVar, sValue, nMemberMaxLimit);
      if (nMemberMaxLimit < 0) then begin
...
begin
  ...
  nTime := Str_ToInt(QuestActionInfo.sParam4, -1);
  if (nPosition < 0) or (nPosition > 7) or (nCount < 0) or (nTime < 0) then begin
...
begin
  ...
  for I := 0 to BaseObject.m_SlaveList.Count - 1 do begin
    ...
    if AObject.m_Abil.Level > nSlaveLevel then nSlaveLevel := AObject.m_Abil.Level;
  end;
