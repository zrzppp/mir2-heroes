...
type
  TFrmOnLineHum = class(TForm)
    ...
    procedure ListViewOnLineKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
...
procedure TFrmOnLineHum.ListViewOnLineKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  ListItem: TListItem;
  nIndex: Integer;
  ...
  case Key of
    Word('F'): begin
        if ssCtrl in Shift then begin
          ...
          for nIndex := 0 to ListViewOnLine.Items.Count - 1 do begin
            ListItem := ListViewOnLine.Items.Item[nIndex];
            if sIPaddr = ListItem.SubItems.Strings[0] then begin
              ListViewOnLine.ItemIndex := nIndex;
              break;
            end;
          end;
        end;
