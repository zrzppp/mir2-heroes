...
{$R *.dfm}
const
  g_sTitle = '飞尔世界真彩图片资源文件 2009/01/01 http://www.941yx.com';
...
begin
  ...
  if not FileExists(sOldFileName) then begin
    Application.MessageBox(PChar(Format('%s 没有发现 ！！！', [sOldFileName])), '提示信息', MB_ICONQUESTION);
    EditOldFile.SetFocus;
    Exit;
  end;
  if CompareText(sOldFileName, sNewFileName) = 0 then begin
    Application.MessageBox('旧文件和新文件路径设置不能是同一个路径！！！', '提示信息', MB_ICONQUESTION);
  ...
  if (DataHeader.BitCount <> 16) or (Pos('2009', DataHeader.Title) > 0) then begin
    Application.MessageBox('请选择正确的旧Data文件 ！！！', '提示信息', MB_ICONQUESTION);
    OldFileStream.Free;
    EditOldFile.SetFocus;
    Exit;
  end;
